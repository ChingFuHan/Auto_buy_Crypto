# Claude Task v2: 歷史資料補齊 + Aggressive Breakout 可審核回測

請依本專案規則執行一個「歷史資料補齊 + aggressive breakout 可審核回測」長任務。

本 v2 任務稿是在 `task_temp.md` 基礎上補強：dry-run estimate、`AGENTS.md` 進場、DB 寫入快照、Layer 2 entry / stop / horizon 定義、conservative Layer 2 決策規則、survivor bias 表述與 candidate-level CSV。
...
4. 不得用「DB 內所有 symbol 近 7 天都有資料」推論「無 delisted bias」。只能說 DB 內現有 symbols 近期仍有資料；歷史已下架 symbols 是否缺失仍需標註為 survivor bias。
5. 輸出盤點摘要到報告，不要只在 terminal 顯示。
...
3. 補資料前必須先做 dry-run estimate：
   ...
   - 預估耗時
   - 預估 DB 寫入量
4. 若 dry-run 預估 API calls、rows 或耗時過大，先停下回報，不直接執行。
5. 優先補：
...
3. Bucket C 的 median return after cost >= 0，或至少不得低於 Baseline median。
