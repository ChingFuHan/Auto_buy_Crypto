# Claude Task v3: 歷史資料補齊 + Aggressive Breakout 可審核回測

請依本專案規則執行一個「歷史資料補齊 + aggressive breakout 可審核回測」長任務。

本 v3 任務稿是在 `task_temp_v2.md` 基礎上補強：schema / unique constraint 驗證、dry-run 硬上限、SignalEngine side-by-side 對照、sanity 數值容差、Layer 2 volume/stop 邊角規則、trigger 欄位、median 決策收斂、multiple-testing 警語與 wall-clock budget。
...
2. 驗證每張目標表的 schema、欄位型別、primary key / unique constraints / indexes，並寫入報告。
...
3. 補資料前必須先做 dry-run estimate：
   ...
   - 預估 DB 寫入量
4. dry-run 硬上限：
   - 預估 API calls > 50,000：停下回報。
   - 預估新增 rows > 2,000,000：停下回報。
   - 預估 wall-clock > 2 小時：停下回報。
   - 任一單表預估新增 rows > 現有 rows 的 50%：停下回報。
5. 若 dry-run 預估超過上述任一硬上限，或你無法可靠估算，先停下回報，不直接執行。
6. 優先補：
   - 15m：盡可能補到 Binance 可取得範圍或專案設定允許的最大範圍，但必須受 dry-run estimate 約束。
   - 3m：至少覆蓋 15m 回測所需期間，用於 Layer 2 in-progress approximation。
7. 每批寫入都必須使用 `ON CONFLICT DO NOTHING`。
8. 寫入前必須先記錄每張目標表的可審核快照，並寫入報告：
   - table
   - before row count
   - before min/max da
...
10. 若遇到 Binance 429 或 API limit：
   - 最多依專案 retry 規則重試。
...
1. 完全複製 `SignalEngine` 的指標邏輯：
   ...
   - atr_pct = avg((hi-lo)/cl) over previous finalized 20 bars；這是 percent ATR，不是標準 ATR。`cl` 必須使用該 finalized bar 自己的 close，逐字對照 `SignalEngine._avg_range_pct()`。
   - range = (max_hi - min_lo) / min_lo over previous finalized 20 bars
   - vol_ratio = current_bar.vol / avg(previous finalized 20 vol)
   - ret = (current_bar.cl - previous_close) / previous_close；`previous_close` 是 current bar 前一根 finalized 15m close，逐字對照 `SignalEngine._bar_return_pct()` 的 reference close。
   - breakout = current_bar.hi > max(previous finalized 12 hi)
   - prior_runup = range over previous finalized 5 bars
   - recent_green = consecutive green bars ending at previous finalized bar；green 定義必須是 `close_price > open_price`，不包含 current bar，逐字對照 `SignalEngine._recent_green_bars()`。
2. 報告必須包含 `SignalEngine` source 與回測腳本實作的 side-by-side 對照，至少覆蓋 atr_pct、range、vol_ratio、ret、breakout、prior_runup、recent_green。
...
- Layer 2 的 stop reference 必須使用觸發當下之前已形成的 3m cumulative low；若做不到，需標示為 lookahead-biased stop，不可用於正式決策。
- 若第一根 3m 子K 即觸發，且沒有更早的 in-progress low 可參照，stop reference 至少必須以 trigger 3m bar 的 open / low 規則明確定義並在報告中列出；不得產生 stop distance = 0 的假訊號。
...
5. DB 表 row count before/after。
6. 資料範圍與 survivor bias 風險。
7. SignalEngine 等價性說明。
8. Sanity checks。
9. Layer 1 / 1.5 / 2 結果表。
...
8. 結果不能只依賴少數切片：若 A/B/C/Baseline × horizons × entry modes 中只有 1-2 個切片為正，視為證據不足；需在多個合理 horizon / entry mode 下穩定為正，才可建議 prototype。
...
- 若 wall-clock 已超過 2 小時仍未完成 Phase 7 報告，或任務明顯超出可控成本，必須停下回報目前完成狀態、阻塞點與下一步，不得繼續長時間無回報執行。
